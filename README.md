...
## Local Aggregation Test Results

The aggregation endpoint was tested locally after the database reached approximately **1,000,000 stored logs**.

The test simultaneously ingested **100,000 new logs** while issuing aggregation requests at approximately **1 request per second**.

### Test Configuration

* **Existing database size:** ~1,000,000 logs
* **Logs ingested during test:** 100,000
* **Aggregation request rate:** ~1 request/second
* **Failed ingestion batches:** 0
* **Aggregation requests:** 16
...
| ------------------------ | -----------: |
| Aggregation Requests     |       **16** |
| Minimum Latency          |  **5.04 ms** |
| Average Latency          | **14.05 ms** |
| P50                      |  **6.14 ms** |
| P95                      | **96.94 ms** |
| P99                      | **96.94 ms** |
| Maximum Latency          | **96.94 ms** |
| Failed Ingestion Batches |        **0** |

### Summary

The aggregation test completed successfully while the database contained approximately **1 million logs**.

Aggregation remained responsive during concurrent ingestion, with:

* **14.05 ms average latency**
* **6.14 ms P50 latency**
* **96.94 ms P95 latency**
* **0 failed ingestion batches**

This demonstrates that aggregation remains performant on a large dataset while new logs are being ingested concurrently.

> **Note:** These are local development results. Benchmark results under enforced resource limits may differ depending on the machine and Docker resource constraints.
